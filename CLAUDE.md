...
- **`pipeline.py` 是核心编排器**。`Pipeline.process_item/process_path/process_target/scan` 串起识别→刮削→落地→写状态库。`build_scrapers/build_notifiers/build_mediaservers` 按配置实例化插件；刮削是**链式**的（依次尝试，首个命中者胜），无刮削器时降级到 `null`（仅按文件名整理）。
