...
def api_login(body: LoginBody, ctx: WebContext = Depends(get_ctx)):
    acc = ctx.cfg().auth
    if body.username != acc.username or not auth.verify_password(body.password, acc.password_hash):
        raise HTTPException(401, "用户名或密码错误")
    return {"token": auth.issue_token(), "username": acc.username}
...
def api_me(ctx: WebContext = Depends(get_ctx)):
    # 能到这里说明已过中间件鉴权
    return {"username": ctx.cfg().auth.username}
